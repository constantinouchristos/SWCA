...
class SWAG(torch.nn.Module):
    def __init__(
        self, base, no_cov_mat=True, max_num_models=0, var_clamp=1e-30, model_args=None, config=None,**kwargs
    ):
        ...
        model_b =  BertModel.from_pretrained(
    ...
    def sample_fullrank(self, scale, cov, fullrank):
        ...
        for i in list(self.base.bert.modules()):

            if isinstance(i,transformers.models.bert.modeling_bert.BertEmbeddings):

                i.__setattr__('position_ids',getattr(i,'position_ids').cuda())
...
class SWAG_adversarial(torch.nn.Module):
    def __init__(
        self, base, no_cov_mat=True, max_num_models=0, var_clamp=1e-30, model_args=None, config=None,**kwargs
    ):
        super(SWAG_adversarial, self).__init__()
        ...
        model_b =  BertModel_c.from_pretrained(
            model_args.model_name_or_path,
            from_tf=bool(".ckpt" in model_args.model_name_or_path),
                config=config,
                cache_dir=model_args.cache_dir
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        super(SWAG_adversarial, self).load_state_dict(state_dict, strict)
